...
public class GrafoPesado {
    ...
    public GrafoPesado(int nroDeVertices) throws ExcepcionesNroVerticeInvalido {
        if (nroDeVertices < 0) {
    ...
    public double obtenerPeso(int origen, int destino) {
        for (AdyacenteConPeso adyacente : this.listaDeAdyacencias.get(origen)) {
            if (adyacente.getIndiceVertice() == destino) {
    ...
    public void eliminarArista(int posDeVerticeOrigen, int posDeVerticeDestino) throws ExcepcionPosicionDelVerticeInvalida, AristaNoExiste {
        ...
        if (posDeVerticeOrigen != posDeVerticeDestino) {
            adyacente = null;
            for (AdyacenteConPeso ady : adyacentesDelDestino) {
                if (ady.getIndiceVertice() == posDeVerticeOrigen) {
                    adyacente = ady;
                    break;
                }
            }
            if (adyacente != null) {
                adyacentesDelDestino.remove(adyacente);
            }
        }
    }
